...
public class NeuralController : MonoBehaviour
{
	...
	public bool lastHit;
	public int autoGoals = 0;
	public float possessionTime = 0.0f;
	public float possession = 0.0f;
	...
	public float GetScore()
	{
		...
		float possessionScore = 100.0f;			// Assigned to 200 per 10% possession starting from 50;
		float possessionPercentage = possession / maxSimulTime * 100;

		float fitness = 0;


		if (possessionPercentage > 50)
		{
			for(int i = 50; i <= 100; i+=10)
			{
				if (possessionPercentage >= i)
					fitness += possessionScore;
				else
					break;
			}
		}
		else if(possessionPercentage < 50)
		{
		...
		fitness += 20 * hitTheBall;
		fitness += -GoalsOnMyGoal * 500 + 500 * GoalsOnAdversaryGoal - 100 * autoGoals;
